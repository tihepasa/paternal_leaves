// spatio-temporal beta-binomial logit regression with explanatory variables and spatial component
data {
...
parameters {
  ...
  // dispersion for beta distribution
  real<lower = 0> kappa;
}
...
model {
  ...
  target += beta_binomial_lpmf(response[ind] | N_all[ind],
  inv_logit(c[time[ind]] + to_vector(sigma_phi * phi)[ind] + rows_dot_product(covariates_t, beta[time, ])[ind]) * kappa,
  (1 - inv_logit(c[time[ind]] + to_vector(sigma_phi * phi)[ind] + rows_dot_product(covariates_t, beta[time, ])[ind])) * kappa);
}
generated quantities {
  ...
  for(i in 1:n_obs) {
    log_lik[i] = beta_binomial_lpmf(response[ind[i]] | N_all[ind[i]],
    inv_logit(c[time[ind[i]]] + to_vector(sigma_phi * phi)[ind[i]] + rows_dot_product(covariates_t, beta[time, ])[ind[i]]) * kappa,
    (1 - inv_logit(c[time[ind[i]]] + to_vector(sigma_phi * phi)[ind[i]] + rows_dot_product(covariates_t, beta[time, ])[ind[i]])) * kappa);
  }
}
