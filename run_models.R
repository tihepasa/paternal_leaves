# Code to estimate the four alternative models (for both datas).
...
mode(response_n) <- mode(N_all) <- "integer"
...
# collect the parameters for the model
...
# fit the model, get 6500 posterior samples
fit <- sampling(model, data = pars, init = inits, iter = 8000, warmup = 1500,
                chains = 4, cores = 4, refresh = 50, save_warmup = FALSE)

# save or inspect the model, for example
print(fit, pars = "sigma_c", use_cache = FALSE)

#saveRDS(fit, "fit_bin_phi_data2.rds")
