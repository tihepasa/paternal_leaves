// spatio-temporal beta-binomial logit regression with explanatory variables
data {
...
model {
  ...
  // assign beta-binomial distribution with logit link
  target += beta_binomial_lpmf(response[ind] | N_all[ind],
  inv_logit(c[time[ind]] + rows_dot_product(covariates_t, beta[time, ])[ind]) * kappa,
  (1 - inv_logit(c[time[ind]] + rows_dot_product(covariates_t, beta[time, ])[ind])) * kappa);
}
generated quantities {
  ...
  for(i in 1:n_obs) {
    ...
    inv_logit(c[time[ind[i]]] + rows_dot_product(covariates_t, beta[time, ])[ind[i]]) * kappa,
    (1 - inv_logit(c[time[ind[i]]] + rows_dot_product(covariates_t, beta[time, ])[ind[i]])) * kappa);
  }
}
